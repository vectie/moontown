...
- keep Cookbook DTOs, artifact summary accounting, required-missing drift
  semantics, deterministic manifest/page content, generated-site path policy,
  and operator-facing Cookbook status Markdown in `cookbook/`; keep artifact
  existence observation, MoonBook catalog registration, manifest persistence,
  workspace writes, generated-site writes, status inspection, and command
  entrypoints in `cookbook_runtime/`; root must not coordinate Cookbook
  filesystem/catalog behavior
...
- keep book-quality score computation, required-relative-path derivation,
  semantic-review parsing, and structural/content observation DTOs in
  `book_quality/`; keep workspace/path existence observation, civic-result
  observation, semantic-review result file reads, research report word-count
  observation, course index text observation, cookbook audit observation, and
